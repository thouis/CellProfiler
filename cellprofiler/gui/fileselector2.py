...
class Clause(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.metadata_choice = wx.Choice(self, choices=['Filename', 'Pathname'])  # defaults
        ...
        self.metadata_keys = self.metadata_choice.Items  # true key in metadata

        self.sizer.Add(self.metadata_choice)
        ...
        self.metadata_choice.Bind(wx.EVT_CHOICE, self.update)
    ...
    def set_focus(self, evt):
        ...
        previous_metadata_key = self.metadata_keys[self.metadata_choice.Selection]
        self.Parent.set_focus(self)  # this will result in refiltering the displayed list.
        # update the metadata choices after asking the parent to update.  This
        # will call self's own mark_matches() method, storing the list of available
        # columns.
        # XXX ... but really, we should regenerate each list completely, so the
        # choices are up to date immediately.
        if not previous_metadata_key in self.metadata_keys:
            # XXX - Probably should warn user.  Especially if the missing data
            # was previously a column generated by a regexp clause
            self.metadata_keys.append(previous_metadata_key)
        self.metadata_choice.Items = [str(k) for k in self.metadata_keys]
        self.metadata_choice.Selection = self.metadata_keys.index(previous_metadata_key)
    ...
    def mark_matches(self, filelist, mark=None):
        try:
            seen_metadata_keys = OrderedDict()
            ...
            metadata_key = self.metadata_keys[self.metadata_choice.Selection]
            for pf in filelist:
                if not pf.get(mark, True):
                    continue
                seen_metadata_keys.update(pf)  # to update metadata columns choices in self.set_focus()
                in_filter = ((inverted and not filt(unicode(pf.get(metadata_key, None)))) or
                             (filt(unicode(pf.get(metadata_key, None))) and not inverted))
                pf[mark] = in_filter
            if mark in seen_metadata_keys:
                del seen_metadata_keys[mark]
            self.metadata_keys = list(seen_metadata_keys)
        except:
            tb = sys.exc_info()[2]
            import pdb
            pdb.post_mortem(tb)
...
class Filter(wx.Panel):
    ...
    def set_focus(self, focused_clause):
        self.Parent.set_focus(self, focused_clause)

    def mark_matches(self, file_list, until_clause=None):
        for clause in self._clauses:
            clause.mark_matches(file_list, mark=self)
            if clause == until_clause:
                break
        return self
...
class FilterSet(wx.Panel):
    ...
    def set_focus(self, focused_filter, focused_clause):
        file_list = [f.copy() for f in self.get_unfiltered_files()]
        for filt in self._filters:
            the_mark = filt.mark_matches(file_list, until_clause=focused_clause)
            if filt == focused_filter:
                file_list = [f for f in file_list if f[the_mark]]
                for f in file_list:
                    del f[the_mark]
                break
        # only show those files which passed the filter, and don't show the current filter's mark?
        self.set_filtered_list(file_list, "foo")
